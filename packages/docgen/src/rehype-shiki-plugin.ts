...
export function rehypeShikiPlugin({
  ...
  return async function transformer(tree) {
    ...
    let hastToString = null

    if (hastToString === null) {
      hastToString = (await import('hast-util-to-string')).toString
    }

    languageCodeBlocks.forEach((node) => {
      highlightBlock(hastToString, highlighter, theme, node)
    })
  }
}
